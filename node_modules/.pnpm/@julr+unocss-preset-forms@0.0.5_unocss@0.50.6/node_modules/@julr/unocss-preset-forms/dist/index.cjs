...
function presetForms() {
  return {
    ...
    preflights: [
      {
        getCSS: ({ theme }) => {
          const spacing = (s) => utils.handler.fraction.rem(`${s}` || "1");
          ...
          const createStyleObject = ([key, value]) => {
            if (typeof value === "object") {
              return Object.entries(value).map((styles) => createStyleObject(styles)).join("\n");
            }
            return `${key}: ${value};`;
          };
          const style = rules.map((rule) => {
            const selector = rule.base.join(", ");
            const styles = Object.entries(rule.styles).map((style2) => createStyleObject(style2)).join("\n");
...
exports.presetForms = presetForms;
