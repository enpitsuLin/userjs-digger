import { noop, resolveUnref, isClient, isString, tryOnScopeDispose, isIOS, tryOnMounted, computedWithControl, promiseTimeout, isFunction, resolveRef, increaseWithUnit, objectEntries, useTimeoutFn, pausableWatch, createEventHook, timestamp, pausableFilter, watchIgnorable, debounceFilter, createFilterWrapper, bypassFilter, createSingletonPromise, toRefs, containsProp, until, hasOwn, throttleFilter, useDebounceFn, useThrottleFn, isObject, isNumber, useIntervalFn, clamp, syncRef, objectPick, tryOnUnmounted, watchWithFilter, identity, isDef } from '@vueuse/shared';
...
function onKeyDown(key, handler, options = {}) {
  return onKeyStroke(key, handler, __spreadProps$a(__spreadValues$o({}, options), { eventName: "keydown" }));
}
function onKeyPressed(key, handler, options = {}) {
  return onKeyStroke(key, handler, __spreadProps$a(__spreadValues$o({}, options), { eventName: "keypress" }));
}
function onKeyUp(key, handler, options = {}) {
  return onKeyStroke(key, handler, __spreadProps$a(__spreadValues$o({}, options), { eventName: "keyup" }));
}
...
function useCssVar(prop, target, options = {}) {
  ...
  watch([elRef, () => resolveUnref(prop)], updateCssVar, { immediate: true });
  watch(variable, (val) => {
    var _a;
...
function useCycleList(list, options) {
  ...
  const index = computed({
    get() {
      var _a;
      const targetList = resolveUnref(list);
  ...
  function getInitialValue() {
    var _a, _b;
    return (_b = resolveUnref((_a = options == null ? void 0 : options.initialValue) != null ? _a : resolveUnref(list)[0])) != null ? _b : void 0;
  }
  ...
  const isSupported = useSupported(() => {
    if (!document) {
      ...
      for (const m of functionsMap) {
        if (m[1] in document || target2 && m[0] in target2) {
          map = m;
          return true;
        }
      }
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
  ...
  const state = reactive(useScroll(element, __spreadProps$3(__spreadValues$9({}, options), {
...
function executeTransition(source, from, to, options = {}) {
  ...
  const duration = (_a = unref(options.duration)) != null ? _a : 1e3;
  const startedAt = Date.now();
  const endAt = Date.now() + duration;
  const trans = (_b = unref(options.transition)) != null ? _b : identity;
...
function useTransition(source, options = {}) {
  ...
  const sourceVal = () => {
    ...
    return isNumber(v) ? v : v.map(resolveUnref);
  };
  ...
  watch(sourceVal, async (to) => {
    ...
    if (id !== currentId)
      return;
    const toVal = Array.isArray(to) ? to.map(resolveUnref) : resolveUnref(to);
...
function useUserMedia(options = {}) {
  ...
  watch([videoDeviceId, audioDeviceId, constraints], () => {
