...
;(function (exports, shared, vueDemi, core) {
  ...
  function useBrowserLocation({ window = defaultWindow } = {}) {
    const refs = Object.fromEntries(WRITABLE_PROPERTIES.map((key) => [key, vueDemi.ref()]));
  ...
  function useCycleList(list, options) {
    ...
    const listRef = shared.resolveRef(list);
    const index = vueDemi.computed({
      get() {
        var _a;
        const targetList = shared.resolveUnref(list);
    ...
    function getInitialValue() {
      var _a, _b;
      return (_b = shared.resolveUnref((_a = options == null ? void 0 : options.initialValue) != null ? _a : shared.resolveUnref(list)[0])) != null ? _b : void 0;
    }
  ...
  function useInfiniteScroll(element, onLoadMore, options = {}) {
    ...
    const state = vueDemi.reactive(useScroll(element, __spreadProps$3(__spreadValues$9({}, options), {
  ...
  function useTextareaAutosize(options) {
    ...
    function triggerResize() {
      ...
      if (options == null ? void 0 : options.styleTarget)
        vueDemi.unref(options.styleTarget).style.height = `${textareaScrollHeight.value}px`;
      else
  ...
  const toVec = (t) => (shared.isNumber(t) ? [t] : t) || [];
  function executeTransition(source, from, to, options = {}) {
    var _a, _b;
    const fromVal = vueDemi.unref(from);
    const toVal = vueDemi.unref(to);
    const v1 = toVec(fromVal);
    const v2 = toVec(toVal);
    const duration = (_a = vueDemi.unref(options.duration)) != null ? _a : 1e3;
    const startedAt = Date.now();
    const endAt = Date.now() + duration;
    const trans = (_b = vueDemi.unref(options.transition)) != null ? _b : shared.identity;
    const ease = shared.isFunction(trans) ? trans : createEasingFunction(trans);
    return new Promise((resolve) => {
      ...
      const tick = () => {
        ...
        else if (shared.isNumber(source.value))
          source.value = arr[0];
        if (now < endAt) {
  ...
  function useTransition(source, options = {}) {
    let currentId = 0;
    const sourceVal = () => {
      const v = shared.resolveUnref(source);
      return shared.isNumber(v) ? v : v.map(shared.resolveUnref);
    };
    const outputRef = vueDemi.ref(sourceVal());
    vueDemi.watch(sourceVal, async (to) => {
      var _a, _b;
      if (vueDemi.unref(options.disabled))
        return;
      const id = ++currentId;
      if (options.delay)
        await shared.promiseTimeout(vueDemi.unref(options.delay));
      if (id !== currentId)
        return;
      const toVal = Array.isArray(to) ? to.map(shared.resolveUnref) : shared.resolveUnref(to);
    ...
    vueDemi.watch(() => vueDemi.unref(options.disabled), (disabled) => {
      if (disabled) {
        currentId++;
    ...
    return vueDemi.computed(() => vueDemi.unref(options.disabled) ? sourceVal() : outputRef.value);
  }
  ...
  function useUserMedia(options = {}) {
    ...
    vueDemi.watch([videoDeviceId, audioDeviceId, constraints], () => {
