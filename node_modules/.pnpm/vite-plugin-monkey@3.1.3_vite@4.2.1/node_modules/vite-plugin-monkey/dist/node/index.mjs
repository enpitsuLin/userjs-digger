...
var externalResourcePlugin = (finalOption) => {
  ...
  return {
    ...
    generateBundle() {
      const usedModIdSet = new Set(
        Array.from(this.getModuleIds()).map((s) => normalizePath5(s))
      );
...
async function* walk(dirPath) {
  const pathnameList = (await fs4.readdir(dirPath)).map(
    (s) => path4.join(dirPath, s)
  );
  while (pathnameList.length > 0) {
    const pathname = pathnameList.pop();
    const state = await fs4.lstat(pathname);
    if (state.isFile()) {
      yield pathname;
    } else if (state.isDirectory()) {
      pathnameList.push(
        ...(await fs4.readdir(pathname)).map((s) => path4.join(pathname, s))
      );
    }
  }
}
...
import { transformWithEsbuild as transformWithEsbuild4 } from "vite";
    ...
    async generateBundle(_, esmBundle) {
      ...
      if (cssList.length > 0) {
        ...
        if (!viteConfig.build.minify && finalOption.build.minifyCss) {
          ...
          css = (await transformWithEsbuild4(css, "any_name.css", {
            sourcemap: false,
            legalComments: "none",
            minify: true,
  ...
  return {
    ...
    async transform(code, id) {
      if (viteConfig.assetsInclude(id) && [`.svg`, `.svg?inline`, `.svg?url`].some((s) => id.endsWith(s)) && code.includes(`__VITE_ASSET__`)) {
        const base64Text = (await fs5.readFile(id.split("?")[0])).toString(
          "base64"
        );
